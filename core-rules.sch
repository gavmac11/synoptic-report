...
<schema queryBinding="xslt2" xmlns="http://purl.oclc.org/dsdl/schematron"
	...
	<p>In this schema, there are multiple occasions when I need to convert a value expressed in
		...
		conversion factor. If the input is cannot be handled, the fallback is to report the result
		as an empty sequence.</p>
	...
	<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
	<p>This is an xslt2 function that returns a descending sort of a supplied sequence of
		decimals.</p>
	...
	<pattern id="specify-element-iff-value-of-specify">
		...
		<p>This pattern ensures that a response value of 'specify' is accompanied by a child
			{specify} element where the user's specify string is reported. The point: you can't
			respond to an item by saying you're going to specify a different response from any of
			the available options, then fail to make such a specification. </p>
		...
		<rule context="ecc:specify">
			<assert
				test="(../attribute::* = 'specify') or (ends-with(../attribute::*,     '(specify)'))"
					>In a "<value-of select="../../@name"
					/>" item, the response contains a
				"specify" element, but the response value is "<value-of
					select="../@value"/>",
				rather than "specify". .</assert>
		</rule>
		<rule context="//ecc:response[attribute::* = 'specify']">
			<assert test="exists (ecc:specify)">Item "<value-of select="../@name"
				/>" has a response
				value of "specify", but no specification ("specify" element) is given.</assert>
		</rule>
	...
	<pattern id="unit-inapplicable">
		<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
		<p>This pattern ensures that if any measurement has either the unit of the value equal to
			inapplicable, then both are inapplicable.</p>
		<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
		<rule context="//ecc:response[@unit]">
			<assert
				test="not('inapplicable' = (@unit, @value)) or not('inapplicable' != (@unit,     @value))"
					>In the "<value-of select="../@name"
				/>" item, an "inapplicable" response
				for either unit or value requires the same response for both.</assert>
		</rule>
	</pattern>
	<!--=============================================================-->
	<pattern id="dimensions-have-same-unit">
		...
		<rule context="//ecc:item[ecc:response[@unit][2]]">
			...
			<report role="warning" test="count(distinct-values($units)) gt 1"
					>You should use the
				same unit for all dimensions of the "<value-of
					select="@name"/>" item.</report>
		</rule>
	</pattern>
	<!--=============================================================-->
	<pattern id="largest-dimension-comes-first">
		...
		<rule context="//ecc:item[ecc:response[@unit][2]]" role="warning">
			<let name="unsorted"
				value="for $x in ecc:response return ecc:to-mm($x/@value, $x/@unit,     $x/@relation)"/>
			<let name="sorted" value="ecc:sort($unsorted)"/>
			<report
				test="not(every $x in (1 to count($sorted)) satisfies $sorted[$x]     eq     $unsorted[$x])"
					>Dimensions should be in descending order in the "<value-of select="@name"
				/>" item.</report>
			<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
		</rule>
	</pattern>
	...
	<pattern id="closest-margin-negative">
		...
		<rule context="//ecc:section[@name eq 'margins']">
			...
			<let name="total-count"
				value="for $x in $margin-type return count($margin[@name eq     $x]/ecc:item[@name                 eq                 'status'])"/>
			...
			<let name="closest-not-unique"
				value="for $x in (1 to $type-count) return     $margin-type[$x][$closest-count[$x] gt 1]"/>
			...
			<assert test="empty($closest-but-not-all-negative)">For margin type(s) "<value-of
					select="string-join($closest-but-not-all-negative, '&quot; and &quot;')"
				/> a
				"closest" margin is specified even though not all margins are negative.</assert>
			<assert test="empty($closest-not-unique)">For margin type(s) "<value-of
					select="string-join($closest-not-unique, '&quot; and &quot;')"
				/>" more than one
				closest margin is claimed.</assert>
			<report role="warning" test="$negative-but-no-closest"
					>All margins are negative for
					"<value-of
					select="string-join($negative-but-no-closest, '&quot; and &quot;')"
				/>"; therefore one margin should be designated as "closest".</report>
		</rule>
	</pattern>
	<!--=============================================================-->
	<pattern id="margin-distance-only-if-negative">
		...
		<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
		<rule
			context="//ecc:section[@name eq 'margin']/ecc:section[ecc:item[@name eq    'status'][ecc:response/@value             eq 'positive']]">
			<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
			<assert test="not(ecc:item[@name eq 'distance'])"
					>Distance from margin may not be
				specified for "<value-of select="@name"
					/>" at the "<value-of
					select="../ecc:item[@name eq 'location']/ecc:response/@value"
				/>" margin because
				that margin is positive.</assert>
		</rule>
	</pattern>
	<!--=============================================================-->
	<pattern id="tumorSite-subsets-specimenSite">
		<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
		<p>This checks that every site that you specify as being involved by tumor is actually a
			site that you also have specified as being contained in the specimen. It would make no
			sense to say that tumor involves the left ear, if the left ear is not listed as part of
			the specimen.</p>
		<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
		<rule context="//ecc:item[@name eq 'tumor sites']">
			<let name="tumorSites" value="ecc:response/@value"/>
			<let name="specimenSites"
				value="//ecc:item[@name eq 'specimen sites']/ecc:response/@value"/>
			<let name="mismatch"
				value="for $x in $tumorSites return if ($x = ($specimenSites,'inapplicable','unreported','specify')) then () else $x"/>
			<assert test="empty($mismatch)">You report tumor at one or more site(s) ("<value-of
					select="string-join($mismatch, '&quot;, ')"/>") not included in the specimen.</assert>
		</rule>
	</pattern>
	<!--=============================================================-->
	<pattern id="tumor-largest-dimension-should-not-exceed-specimen-largest-dimension">
		...
		<rule context="//ecc:section[@name eq 'specimen']/ecc:item[@name eq 'tumor size']">
			...
			<report test="$maxTumorDimension gt $maxSpecimenDimension"
					>Largest gross tumor dimension
					(<value-of select="$maxTumorDimension"
					/> mm) exceeds specimen largest dimension
					(<value-of
	...
	<pattern id="non-negative-nodes-cannot-exceed-total-nodes">
		...
		<rule context="//ecc:section[@name eq 'tally']">
			<let name="total"
				value="ecc:item[@name eq 'total']/ecc:response/@value[. castable as     xs:decimal]"/>
			<let name="other"
				value="ecc:item[@name ne 'total']/ecc:response/@value[. castable as     xs:decimal]"/>
			...
			<assert test="if (exists($total)) then number($total) ge sum($other) else $skip"
					...
					/>" group, you report fewer total nodes (<value-of select="$total"
					/>) than the sum of the enumerated node status categories
					(<value-of
	...
	<pattern id="clinical-treatment-and-pathologic-effect-consonant">
		...
		<rule context="/ecc:section">
			<let name="pathologic"
				value="ecc:section[@name eq 'accessory']/ecc:item[@name eq     'treatment                 effect']/ecc:response/@value"/>
			<let name="clinical"
				value="ecc:section[@name eq 'clinical']/ecc:item [@name eq 'prior     therapies']/ecc:response/@value"/>
			<report role="warning"
				test="exists($pathologic) and $pathologic !=     ('inapplicable','unreported') and                 empty($clinical)"
				>You report
				treatment effect among the accessory findings, but there is no prior therapy report
				in the clinical section.</report>
		</rule>
	</pattern>
	<!--=============================================================-->
	<pattern id="grade-comports-with-grade-system">
		...
		<rule context="//ecc:item[@name eq 'histologic grade']/ecc:response">
			<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
			<let name="ignore"
				value="('specify', 'unreported', 'equivocal', 'indeterminate',     'inapplicable')"/>
			<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
			<assert
				test="if (@value = $ignore) then $skip else if (@system eq '2-grade') then     @value =                 ('low','high') else if (@system eq '3-grade') then @value =     ('well-differentiated',                 'moderately-differentiated',     'poorly-differentiated') else if (@system eq '4-grade') then @value =     ('GX',     'G1', 'G2', 'G3', 'G4') else $skip"
					>A grade of "<value-of select="@value"/>" is
				incompatible with a "<value-of
					select="@system"/>" system.</assert>
		</rule>
	</pattern>
	<!--=============================================================-->
	<pattern id="response-unique">
		<p>Some queries permit multiple responses. No repeats of the same response are allowed
			except in sizes.</p>
		<rule context="//ecc:item[not(ecc:response/@unit = ('m', 'cm', 'mm', 'in'))]" role="info">
			<let name="response"
				value="ecc:response/@value[. ne 'specify'][. ne 'inapplicable'][.     ne 'unreported']"/>
