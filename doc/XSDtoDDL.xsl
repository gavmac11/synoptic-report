...
<xsl:stylesheet version="1.0" xmlns:msdata="urn:schemas-microsoft-com:xml-msdata"
  ...
  -->
	...
	<xsl:template name="Types">
		...
		<xsl:choose>
			...
			<xsl:when test="substring-after($type, ':')='string'">varchar<xsl:if test="local-name()!='restriction'"
					>(255)</xsl:if></xsl:when>
    ...
    type will eventually reach an xsl: data type -->
			<xsl:otherwise>
				<xsl:apply-templates mode="derivedType" select="$contents//xs:simpleType[@name=$type]"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:template match="xs:maxLength" mode="derivedType">(<xsl:value-of select="@value"/>)</xsl:template>
	<!-- Create foreign keys.
...
-->
	<xsl:template match="xs:keyref[@refer]" name="ForeignKey">
		<!-- variables -->
		...
		<xsl:variable name="parentTable"
			select="substring-after(//xs:key[@name=$parentKeyName][1]/xs:selector[1]/@xpath, './/mstns:')"/>
		<!-- Opening keywords -->
