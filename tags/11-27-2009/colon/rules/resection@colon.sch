...
<schema queryBinding="xslt2" xmlns="http://purl.oclc.org/dsdl/schematron"
   ...
   <pattern id="colon-site-matches-procedure">
      <p>This pattern checks that the reported procedures and reported specimen sites match
         properly. The challenges here are that: (i) We allow both multiple sites and multiple
         procedures to be reported. So we don't want to claim a wrong site just because of a
         mismatch with only a single procedure; and vice versa. And (ii) typically, a given
         procedure has more *possible* sites than *necessary* sites. And a given site is usually a
         match for more than one procedure. </p>
      <p> This is a long, but "dumb" rule. Most of it is data setup. It fires once, when the
         processor encounters a "specimen" section.</p>
      <p> The first thing it does is to set up a buch of temporary variables. These are convenience
         shortnames for all the possible procedure strings and site strings. These strings are
         identical to those allowed by the RNG as responses to the {specimenSite} and {procedure}
         queries; these are obtained by checking the valuesets.rng file. A good enhancement of this
         rule would be to reference those strings directly via an xpath external document reference;
         this would prevent problems of maintaining this schematron file in synch with the RNG
         model. However, for now this is not done and we will have to manually ensure that this list
         is in synch with the valuesets.rng file.</p>
      <p>The next thing is to set up lists of "necessary" and "sufficient" matches between sites and
         procedures (see below). The "sufficient" ones match site-to-procedure. The "necessary" ones
         match procedure-to-site. (For why this choice, see challenge (ii) in the first paragraph
         above.)</p>
      <p>The final thing is to evaluate two assertions. The first assertion insists that for every
         site, there is at least one procedural match in the patient's report. The second assertion
         insists that for every procedure, the minimally matching set of site(s) is on the patient's
         report.</p>
      <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
      <rule context="/ecc:synopsis/ecc:specimen">
         ...
         <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
         <assert test="not($anu = $sites and not($procs = $anu-procs))">Anus is among the reported
            sites, but no matching procedure is reported.</assert>
         <assert test="not($rec = $sites and not($procs = $rec-procs))">Rectum is among the reported
            sites, but no matching procedure is reported.</assert>
         <assert test="not($rsj = $sites and not($procs = $rsj-procs))">Rectosigmoid junction is
            among the reported sites, but no matching procedure is reported.</assert>
         <assert test="not($sig = $sites and not($procs = $sig-procs))">Sigmoid colon is among the
            reported sites, but no matching procedure is reported.</assert>
         <assert test="not($des = $sites and not($procs = $des-procs))">left colon is among the
            reported sites, but no matching procedure is reported.</assert>
         <assert test="not($spl = $sites and not($procs = $spl-procs))">Splenic flexure is among the
            reported sites, but no matching procedure is reported.</assert>
         <assert test="not($tra = $sites and not($procs = $tra-procs))">Transverse colon is among
            the reported sites, but no matching procedure is reported.</assert>
         <assert test="not($hep = $sites and not($procs = $hep-procs))">Hepatic flexure is among the
            reported sites, but no matching procedure is reported.</assert>
         <assert test="not($asc = $sites and not($procs = $asc-procs))">right colon is among the
            reported sites, but no matching procedure is reported.</assert>
         <assert test="not($cec = $sites and not($procs = $cec-procs))">Cecum is among the reported
            sites, but no matching procedure is reported.</assert>
         <assert test="not($ter = $sites and not($procs = $ter-procs))">Terminal ileum is among the
            reported sites, but no matching procedure is reported.</assert>
         <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
         <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
         ...
         <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
         <assert test="not($tox = $procs and not(every $s in $tox-sites satisfies $s = $sites))"
            >Total colectomy is reported, but corresponding specimen sites are absent. </assert>
         <assert test="not($prx = $procs and not(every $s in $prx-sites satisfies $s = $sites))"
            >Proctocolectomy is reported, but corresponding specimen sites are absent.</assert>
         <assert test="not($apx = $procs and not(every $s in $apx-sites satisfies $s = $sites))"
            >Abdominoperineal resection is reported, but corresponding specimen sites are
            absent.</assert>
         <assert test="not($lax = $procs and not(every $s in $lax-sites satisfies $s = $sites))">Low
            anterior resection is reported, but corresponding specimen sites are absent.</assert>
         <assert test="not($tdx = $procs and not(every $s in $tdx-sites satisfies $s = $sites))"
            >Transanal disk excision is reported, but corresponding specimen sites are
            absent.</assert>
         <assert test="not($six = $procs and not(every $s in $six-sites satisfies $s = $sites))"
            >Sigmoidectomy is reported, but corresponding specimen sites are absent.</assert>
         <assert test="not($lhx = $procs and not(every $s in $lhx-sites satisfies $s = $sites))"
            >Left hemicolectomy is reported, but corresponding specimen sites are absent.</assert>
         <assert test="not($trx = $procs and not(every $s in $trx-sites satisfies $s = $sites))"
            >Transverse colectomy is reported, but corresponding specimen sites are absent.</assert>
         <assert test="not($rhx = $procs and not(every $s in $rhx-sites satisfies $s = $sites))"
            >Right hemicolectomy is reported, but corresponding specimen sites are absent.</assert>
         <assert test="not($asx = $sites and not(every $s in $asx-sites satisfies $s = $sites))"
            >Ascending colectomy is reported, but corresponding specimen sites are absent.</assert>
      </rule>
      <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   </pattern>
   <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   ...
   <pattern id="colon-calculate-T-stage">
      ...
      <rule context="/ecc:synopsis/ecc:extent/colon:invasion/ecc:response">
         ...
         <report test="$T-reported ne $T-calculated"> Reported T-stage (T<value-of
               select="$T-reported"/>) does not match calculated (T<value-of select="$T-calculated"
            />).</report>
      </rule>
      <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   </pattern>
   ...
   <pattern id="colon-calculate-N-stage">
      <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
      <p>This pattern checks that the pN assignment is correct, given the node statuses reported in
         the "nodes" section of the patient's report. This is the AJCC 7th edition algorithm.</p>
      <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
      <rule context="/ecc:synopsis/ecc:nodes">
         ...
         <report test="$N-reported ne $N-calculated"> Reported N-stage (N<value-of
               select="$N-reported"/>) does not match calculated (N<value-of select="$N-calculated"
            />).</report>
      </rule>
      <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   </pattern>
   ...
   <pattern>
      ...
      <p>This pattern checks that the pM assignment is correct, given the metastasis statuses
         reported in the "metastases" section of the patient's report. This is the AJCC 7th edition
         algorithm.</p>
      <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   ...
   <pattern id="polyp-iff-polypectomy">
      <rule context="/ecc:synopsis/ecc:specimen">
         <assert
            test="if (exists(colon:polyp)) then ecc:procedure/ecc:response/@value = 'polypectomy' else $skip"
            > Polyp ("colon:polypectomy" element) is reported in the specimen section but the
            procedure is not polypectomy.</assert>
         <assert
            test="if (ecc:procedure/ecc:response/@value = 'polypectomy') then exists(colon:polyp) else $skip"
            > If the procedure is polypectomy then polyp features must be reported
   ...
   <diagnostics>
      <diagnostic id="grade-dxic"> You have specified a <value-of select="@system"/> system.
      </diagnostic>
   </diagnostics>
   <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
