...
<schema queryBinding="xslt2" xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:xi="http://www.w3.org/2001/XInclude"
    ...
    <pattern id="largest-dimension-comes-first">
        ...
        <rule context="//ecc:*[ecc:response[@unit][2]]" role="warning">
            ...
            <assert test="max($value) eq $value[1] and (min($value) eq $value[last()])">   
                Dimensions should be in descending order. Dimensions with non-numeric values should be reported last.
            </assert>
        </rule>
    </pattern>
    <!--=============================================================-->
    <pattern id="margins">
        ...
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <let name="in-situ" value="//ecc:margin/ecc:in-situ/ecc:status/ecc:response"/>
        <let name="invasive" value="//ecc:margin/ecc:invasive/ecc:status/ecc:response"/>
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <rule context="ecc:margins" role="warning">
            <let name="name" value="distinct-values(ecc:margin/@name)"/>
            ...
            <assert role="error" test="count(ecc:margin) eq count($name)">
    ...
    <pattern id="margin-modifier">
        ...
        <rule context="//ecc:margin//ecc:status/ecc:response[@closest|@extent]">
            <assert role="error" test="not(@value = ('indeterminate', $noReport))">
    ...
    <pattern id="non-negative-nodes-cannot-exceed-total-nodes">
        ...
        <rule context="ecc:nodeGroup">
            <let name="totalNodes"
                ...
                xs:integer])"/>
            <let name="otherNodes"
                value="sum(ecc:tally/ecc:response[@type ne $total]/@value[. castable as xs:integer])"/>
    ...
    <pattern id="requires-positive-nodes">
        ...
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <rule context="ecc:nodeGroup">
            <assert test="if (exists(ecc:largestMetastasis) or exists(ecc:extracapsularExtension)) then
            (sum(ecc:tally/ecc:response[@type eq $positive]/@value[. castable as xs:integer]) gt 0) else $skip">
                You may not specify largest metastasis or extracapsular extension unless at least one node in the group is positive.
            </assert>
        </rule>
    </pattern>
    <!--=============================================================-->
    <pattern id="total-nodes-required">
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <p/>
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <rule context="ecc:nodeGroup/ecc:tally">
            <assert test="ecc:response/@type = 'total'">
                You must specify the total number of nodes in the "<value-of select="./../../@name"/>" node group.
            </assert>
        </rule>
    </pattern>
    <!--=============================================================-->
    ...
    <pattern id="response-unique">
        ...
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <rule context="//ecc:*[not(ecc:response/@unit)]" role="warning">
            <let name="response" value="ecc:response[not(@type)]/@value[not(. = $specify)]"/>
