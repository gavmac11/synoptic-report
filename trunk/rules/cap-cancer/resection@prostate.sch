...
<schema queryBinding="xslt2" xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <pattern id="extraprostatic-specified-both-sides">
        <rule context="//prostate:extension">
            ...
            <assert test="$loc = 'right side' and $loc = 'left side'"
                > Extraprostatic extension must be reported for both right
    ...
    <pattern id="seminal-vesicle-specified-both-sides">
        <rule context="//prostate:extension">
            ...
            <assert test="$lat = 'right' and $lat = 'left'"
                > Seminal vesicle extension must be reported for both right and left
                sides. </assert>
        </rule>
    </pattern>
    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
    ...
    <pattern id="calculate-T-stage">
        <rule context="//pert:T">
            ...
            <assert test="$T-reported = $T-calculated"> Reported T-stage (T<value-of select="$T-reported"
                    />) does not match
...
-->
    <pattern id="calculate-N-stage">
        <rule context="//pert:N">
            ...
            <assert test="if ($posNodes eq 0) then $N eq 0 else $N eq 1"> Reported N-stage (N<value-of select="$N"
                    />) does not
                match calculated (N<value-of select="if ($posNodes eq 0) then '0' else '1'"
                />). </assert>
        </rule>
    </pattern>
    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
