...
<schema queryBinding="xslt2" xmlns="http://purl.oclc.org/dsdl/schematron"
    ...
    <pattern id="therapy-description-not-allowed">
        <rule context="//hasPriorTherapy" subject="@description @value">
            <assert test="@value ge count(@description)"> Prior therapy description is forbidden because prior therapy value is
                not "true". </assert>
        </rule>
    </pattern>
    ...
    <pattern id="node-numbers-add-up">
        <rule context="//pert:nodeGroup">
            ...
            <assert test="$itc le $total"> Nodes positive for isolated tumor cells (<value-of select="$itc"/>) must not exceed
                total nodes (<value-of select="$total"/>) in the "<value-of select="@location"/>" lymph node group. </assert>
            <assert test="$micrometastasis le $total"> Nodes positive for micrometastasis (<value-of select="$micrometastasis"/>)
                must not exceed total nodes (<value-of select="$total"/>) in the "<value-of select="@location"/>" lymph node
                group. </assert>
            <assert test="$macrometastasis le $total"> Nodes positive for macrometastasis (<value-of select="$macrometastasis"/>)
                must not exceed total nodes (<value-of select="$total"/>) in the "<value-of select="@location"/>" lymph node
                group. </assert>
            <assert test="$regressed + $itc + $micrometastasis + $macrometastasis le $total"> Sum (<value-of
                    ...
                    select="$micrometastasis"/>) and macrometastasis (<value-of select="$macrometastasis"/>) must not exceed total
    ...
    <pattern id="calculate-N-stage">
        <rule context="//pert:N" subject="@value">
            ...
            <assert test="$N eq $calculated-N">
                Reported N-stage (<value-of select="$N"/>) does not match calculated (<value-of select="$calculated-N"/>). 
            </assert>
        </rule>
    </pattern>
    <pattern id="calculate-T-stage">
        <rule context="//pert:T" id="validate-T-stage">
            ...
            <assert
                ...
                > Reported T-stage (<value-of select="$T"/>) does not match calculated (<value-of
                    select="
                     if ($inflam)           then '4d'
    ...
    <pattern>
        <rule context="//pert:nodeStatus">
            <assert test="if (exists(pert:largestMet)) then @value = ('micrometastasis', 'macrometastasis') else $skip"> A largest
                metastasis is specified for a node group that does not have micro- or macrometastasis. </assert>
            <assert test="if (@value = ('micrometastasis', 'macrometastasis')) then exists(pert:largestMet) else $skip"> Micro- or
