...
<schema xmlns="http://purl.oclc.org/dsdl/schematron">
    ...
    <pattern abstract="true" id="a-subsets-b">
        <rule context="$context">
            <assert test="$a = $b or $escape"> Every <value-of select="$a-name"/> must be <value-of select="$b-name"/>
            </assert>
        </rule>
    </pattern>
    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
    ...
    <pattern abstract="true" id="dimensions-decreasing">
        <rule context="$context">
            ...
            <assert test="$d1 ge $d2 and $d2 ge $d3"> In <name/>, size dimensions must be in decreasing order. </assert>
        </rule>
    </pattern>
    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
