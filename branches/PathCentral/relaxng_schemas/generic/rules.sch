...
<schema queryBinding="xslt2" xmlns="http://purl.oclc.org/dsdl/schematron"
    ...
    <let name="nonspecific" value="($noReport, $specify)"/>
    ...
    <pattern id="margin">
        ...
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <let name="in-situ" value="//ecc:margin//ecc:result[@for eq $insitu]/ecc:response"/>
        <let name="invasive" value="//ecc:margin//ecc:result[@for eq $invas]/ecc:response"/>
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <rule context="ecc:margin">
            <let name="in-situ-closest" value="count($in-situ[@closest eq $positive])"/>
            <let name="in-situ-positive" value="count($in-situ[@value eq $positive])"/>
            <let name="invasive-closest" value="count($invasive[@closest eq $positive])"/>
            <let name="invasive-positive" value="count($invasive[@value eq $positive])"/>
            <report role="error" test="$in-situ-closest gt 0 and $in-situ-positive gt 0">
                A margin is reported as "closest" for in-situ carcinoma, yet at least one margin is positive for in-situ carcinoma.
            </report>
            <report role="error" test="$invasive-closest gt 0 and $invasive-positive gt 0">
                A margin is reported as "closest" for invasive carcinoma, yet at least one margin is positive for invasive carcinoma.
            </report>
            <report role="warning" test="exists($in-situ) and $in-situ-positive eq 0 and $in-situ-closest eq 0">
            ...
            <report role="warning" test="exists($invasive) and $invasive-positive eq 0 and $invasive-closest eq 0">
        ...
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <rule context="ecc:margin//ecc:distance">
            <let name="invas" value=".[@for eq 'invasive']"/>
            <let name="insitu" value=".[@for eq 'in-situ']"/>
            <report test="$invas/../ecc:result[@for eq 'invasive']/ecc:response/@value = ($positive, $noReport)">
                You may not specify the distance to a positive, unreported or "inapplicable" margin.
            </report>
            <report test="$insitu/../ecc:result[@for eq 'in-situ']/ecc:response/@value = ($positive, $noReport)">
                You may not specify the distance to a positive, unreported or "inapplicable" margin.
            </report>
        </rule>
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <rule context="ecc:margin//ecc:response[@closest]">
            <report test="@value eq $positive">
                You may not specify the "closest" property for a positive margin.
            </report>
        </rule>
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <rule context="ecc:margin//ecc:response[@extent]">
    ...
    <!--=============================================================-->
    <pattern abstract="true" id="closest-distance">
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <p>Tests whether the margin desginated as "closest" has the smallest distance specified.</p>
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <rule context="ecc:margin//ecc:result[@for eq $for]/ecc:response[@closest eq $positive]">
            <let name="this" value="../../ecc:distance[@for eq $for]/ecc:response"/>
            <let name="this-mm" value="f:mm($this/@value,$this/@unit,$this/@relation)"/>
            <let name="all-mm"
                value="for $x in //ecc:margin//ecc:distance[@for eq $for]/ecc:response return f:mm($x/@value,$x/@unit,$x/@relation)"/>
            <assert role="error" test="$this-mm le min($all-mm)">
                <value-of select="$this-mm"/>///<value-of select="$all-mm"/>
                The margin designated as "closest" for <value-of select="$for"/> neoplasia does not have the smallest reported distance from the margin."/>
			</assert>
        </rule>
    </pattern>
    <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
    ...
    <!--=============================================================-->
    <include href="shared/subset-rule.sch"/>
    <!--<pattern abstract="true" id="subset-rule">
        <rule context="$where" role="warning">
            ...
            <let name="explosions" value="document('')//data:explosions[@of eq $explode]/data:explosion"/>
            <let name="r"
    ...
    <pattern id="tumor-largest-dimension-should-not-exceed-specimen-largest-dimension">
        ...
        <rule context="ecc:tumor">
            <let name="maxTumorDimension"
                value="max(for $x in .//ecc:size/ecc:response return f:mm($x/@value,$x/@unit,$x/@relation))"/>
            <let name="maxSpecimenDimension"
                value="max(for $x in //ecc:specimen/ecc:size/ecc:response return f:mm($x/@value, $x/@unit,$x/@relation))"/>
    ...
    <pattern id="largest-requires-positive-nodes">
        ...
        <p/>
        <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
        <rule context="ecc:nodes/ecc:status">
            <assert test="if (exists(ecc:largestMetastasis)) then (sum(ecc:result[@for eq $positive]/ecc:response/@value) gt 0) else $skip">
    ...
    <pattern id="ece-requires-positive-nodes">
        <rule context="ecc:nodes/ecc:status">
            <assert test="if (exists(ecc:extracapsularExtension)) then (sum(ecc:result[@for eq $positive]/ecc:response/@value) gt 0) else $skip">
    ...
    <pattern id="node-response-type-appropriate">
        <rule context="ecc:nodes/ecc:status/ecc:result">
    ...
    <pattern id="grade-appropriate-for-grade-system">
        ...
        <rule context="//ecc:tumor//ecc:grade">
            <assert test="false() != (for $x in . return 
                     if ($x/@system eq '2-grade') then $x/ecc:response/@value = $grades-2 
                else if ($x/@system eq '3-grade') then $x/ecc:response/@value = $grades-3 
                else if ($x/@system eq '4-grade') then $x/ecc:response/@value = $grades-4 
                else $skip)">
