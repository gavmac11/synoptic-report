...
<schema queryBinding="xslt2" xmlns="http://purl.oclc.org/dsdl/schematron">
   ...
   <pattern id="total-nodes-must-be-specified-for-each-nodeGroup">
      <rule context="//pert:nodeGroup">
         <assert test="exists(pert:nodeStatus[@value = 'total'])"> At a
                minimum, the total number of nodes in every group must be
                specified (missing for "<value-of select="@location"/>").
            </assert>
      </rule>
   </pattern>
   <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   <pattern>
      <rule context="//*">
         <assert test="if (@value = 'other') then exists(@otherValue) else not(exists(@otherValue))"> In <name/>, a value of 'other" requires a specified
                'otherValue', and vice versa. </assert>
      </rule>
   </pattern>
   <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   <pattern id="node-numbers-add-up">
      <rule context="//pert:nodeGroup">
         ...
         <assert test="$regressed le $total"> Nodes positive for regressed
                ...
                nodes (<value-of select="$total"/>) in the "<value-of select="@location"/>" lymph node group. </assert>
         <assert test="$pos le $total"> Nodes positive (<value-of select="$pos"/>) must not exceed total nodes (<value-of select="$total"/>) in the "<value-of select="@location"/>"
         ...
         <assert test="$regressed + $pos le $total"> Sum (<value-of select="$regressed + $pos"/>) of nodes with regressed tumor
                    (<value-of select="$regressed"/>) and positive (<value-of select="$pos"/>) must not exceed total nodes (<value-of select="$total"/>) in the "<value-of select="@location"/>"
                lymph node group. </assert>
      </rule>
   </pattern>
   <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   <pattern id="mucinous-percentage-only-if-present">
      <rule context="//colon:mucinousComponent">
         <assert test="if (@value ne 'positive') then empty (@percentage) else $skip"> Mucinous component must be positive to specify a percentage.
            </assert>
      </rule>
   </pattern>
   <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   ...
   <pattern id="colon-tumor-site-subsets-specimen-site">
      <rule context="//pert:tumorLocation">
         <assert test="@value = //pert:site/@value"> The tumor site
                    "<value-of select="@value"/>" must be among the sites
   ...
   <pattern id="colon-calculate-N-stage">
      <rule context="//pert:N">
         ...
         <assert test="
                ...
                else                          $N eq 2"> Reported N-stage (<value-of select="$N"/>) does not match
   ...
   <pattern id="adjacent-structure-involvement">
      <rule context="//pert:invasion">
         <assert test="if (colon:deepestInvasion/@value eq 'adjacent structure') then colon:adjacentStructure else $skip"> Deepest invasion of adjacent structure(s) requires specifying
                which structure(s) and extent. </assert>
         <assert test="if (colon:adjacentStructure) then colon:deepestInvasion eq 'adjacent structure' else $skip"> Adjacent structure(s) must be specified only if deepest
   ...
   <pattern id="report-closest-if-all-margins-negative">
      <rule context="//pert:margins">
         ...
         <assert test="if (exists($status) and $status = 'positive') then not(exists($closest)) else $skip"> Closest margin is not reportable if any margin is frankly
                positive. </assert>
         <assert test="if (exists($status) and not($status = 'positive')) then exists($closest) else $skip"> "Closest margin" must be reported if all margins are negative. </assert>
         <assert test="if (exists($closest)) then $closest/@status = 'negative' else $skip"> Closest margin must be negative margin. </assert>
      </rule>
   </pattern>
   <!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
   <pattern>
