...
-->
<schema queryBinding="xslt2" xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:a="http://relaxng.org/ns/compatibility/annotations/1.0">
	...
	<pattern id="node-numbers-add-up">
		<rule context="//pert:nodeGroup">
			<let name="regressed"
				value="
                if (pert:nodeStatus[@value = 'necroinflammatory changes']/@count castable as xs:integer)
                then xs:integer(pert:nodeStatus[@value = 'necroinflammatory changes']/@count)
                else 0
            "/>
			<let name="itc"
				value="
			...
			<assert test="$regressed le $total"
				> Nodes positive for regressed
                tumor (<value-of select="$regressed"/>) must not exceed total
                nodes (<value-of select="$total"/>) in the "<value-of select="@site"/>" lymph node group. </assert>
			<assert test="$itc le $total"
				> Nodes positive for isolated tumor
                cells (<value-of select="$itc"/>) must not exceed total nodes
                    (<value-of select="$total"/>) in the "<value-of select="@site"/>" lymph node group. </assert>
			<assert test="$micrometastasis le $total"
                ...
                exceed total nodes (<value-of select="$total"/>) in the
                    "<value-of select="@site"/>" lymph node group. </assert>
			...
			<assert test="$regressed + $itc + $micrometastasis + $macrometastasis le $total"
				> Sum (<value-of select="$regressed + $itc + $micrometastasis + $macrometastasis"/>) of nodes with regressed tumor (<value-of select="$regressed"/>), isolated tumor cells (<value-of select="$itc"/>),
	...
	<pattern id="calculate-N-stage">
		<rule context="//pert:N" subject="@value">
			...
			<let name="micro" value="sum(//pert:nodeStatus[@value = $mic]/@count)"/>
